...
describe('Bag', () => {
  ...
  it('should not be affected by changes to the passed array of tokens', () => {
    ...
    expect(someTokens).to.deep.equal([Token.ZERO, Token.MINUS_ONE, Token.AUTOFAIL]);
    expect(bag.getTokens()).to.deep.equal([Token.ZERO, Token.MINUS_ONE]);
  });
  ...
  describe('getTokens', () => {
    ...
    it('modifications to the returned array should not affect the Bag', () => {
      ...
      expect(tokens).to.deep.equal([Token.ELDER_SIGN, Token.AUTOFAIL, Token.TABLET]);
  ...
  describe('addTokens', () => {
    ...
    it('should add the tokens', () => {
      ...
      expect(added.getTokens()).to.deep.equal([Token.ELDER_SIGN, Token.AUTOFAIL, Token.TABLET]);
    });
